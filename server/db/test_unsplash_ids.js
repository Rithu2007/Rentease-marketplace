...
const { updateAllImages } = require('./update_images');
...
function testUrl(id) {
  return new Promise((resolve) => {
    const url = `https://images.unsplash.com/${id}?w=10`;
    http.get(url, (res) => {
...
async function run() {
  ...
  for (const [category, ids] of Object.entries(categoryImages)) {
    for (const id of ids) {
      const status = await testUrl(id);
      if (status !== 200) {
        console.log(`[DEAD ID] Category: ${category}, ID: ${id}, Status: ${status}`);
      }
    }
  }
